...
class Trainer:
    ...
    def _setup_logging(self):
        ...
        if logging_config.get('use_wandb', False):
            ...
            # Log system info to wandb.summary (not step-based)
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if self.use_wandb and batch_idx % 100 == 0 and batch_idx > 0:
    ...
    def log_sample_predictions(self, num_samples: int = 2):
        ...
        for i in range(min(num_samples, inputs_np.shape[0])):
            ...
            caption = f"{subject_names[i] if isinstance(subject_names, list) else subject_names[i]} (input/output/target)"
            images.append(wandb.Image(stacked, caption=caption))
        
        # Log with current global step
